...
<svelte:head>
	<title>Mengenal Angka | {APP_NAME}</title>
</svelte:head>
...
<section class="h-full w-full flex bg-white/[.5] backdrop-blur-md">
	<div class="w-full h-full flex bg-[#ddf4e9]/[0.75]" in:fade={{ duration: 150 }}>
		{#if !$mobile}
			<Sidebar number />
			<div class="p-10 text-lg w-full"><LetterItem letter={activeNumber} /></div>
		{:else}
			{#if sidebarOpen}
			...
			<div class="flex-col flex w-full">
				...
				<div class="w-full text-center font-semibold text-xl mb-2">
					<LetterItem letter={activeNumber} />
				</div>
			</div>
		{/if}
	</div>
